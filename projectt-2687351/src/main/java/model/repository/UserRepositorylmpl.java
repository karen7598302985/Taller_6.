...
public class UserRepositorylmpl {
    //parametro =?
    ...
    public class UserRepositoryImpl implements  Repository<User>{
        ...
        public User byIdObj(Integer id) throws SQLException {
            sql ="select u.user_id, u.user_firstname, u.user_lastname, u.user_email" + "from users_tbl u where u.user_id=?";
            User user =null;
            ...
            public Integer saveObj(User user) throws SQLException {
                ...
                if (user.getUser_id()!=null && user.getUser_id()>0){
                    sql = "update users_tbl set user_firstname=?, user_lastname=?,user_email=?," + "user_password =aes_encrypt(?,)" +"where user_id=?";
                } else {
                    sql="insert into users_tbl (user_firstname, user_lastname, user_email, user_password)" + "values(upper((?), upper(?), lower(?), "+ aes_encrypt(?,'$2a$12$AxY05V.J8VognZln2uvqNegJ3N6lTu3iZhaoecb7JTQczsUEL6uB'))";
                }
        ...
        public User createObj(ResultSet rs) throws SQLException {
            ...
            user .getUser_lastname(rs.getString("user_lastname"));
            user .getUser_email(rs.getString("user.email"));
            user .getUser_password(rs.getString("user_password"));
            return null;
        }
    }
